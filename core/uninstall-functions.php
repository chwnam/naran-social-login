...
if ( ! function_exists( 'nsl_cleanup_meta' ) ) {
	/**
	 * Remove plugin's meta records.
	 */
	function nsl_cleanup_meta() {
		global $wpdb;
...
if ( ! function_exists( 'nsl_cleanup_option' ) ) {
	/**
	 * Remove plugin's option records.
	 */
	function nsl_cleanup_option() {
		global $wpdb;

		$option_names = [];

		$option = nsl()->registers->option;
		if ( $option ) {
			foreach ( $option->get_items() as $item ) {
				if ( $item instanceof NSL_Reg_Option ) {
...
if ( ! function_exists( 'nsl_cleanup_terms' ) ) {
	/**
	 * Remove plugin's terms.
	 */
	function nsl_cleanup_terms() {
		global $wpdb;

		$taxonomies = [];

		$taxonomy = nsl()->registers->taxonomy;
		if ( $taxonomy ) {
...
if ( ! function_exists( 'nsl_cleanup_posts' ) ) {
	/**
	 * Remove plugin's posts.
	 */
	function nsl_cleanup_posts() {
		global $wpdb;

		$post_types = [];

		$post_type = nsl()->registers->post_type;
		if ( $post_type ) {
