...
if ( ! class_exists( 'NSL_Register_Base_Shortcode' ) ) {
	abstract class NSL_Register_Base_Shortcode implements NSL_Register {
		use NSL_Hook_Impl;
		...
		public function register() {
			foreach ( $this->get_items() as $item ) {
				if ( $item instanceof NSL_Reg_Shortcode ) {
		...
		public function heading_actions_handler() {
			if ( is_singular() && $this->heading_actions ) {
				...
				foreach ( array_unique( $this->found_tags ) as $tag ) {
					$callback = nsl_parse_callback( $this->heading_actions[ $tag ] );
		...
		public function dispatch( $atts, string $enclosed, string $tag ): string {
			$callback = nsl_parse_callback( $this->real_callbacks[ $tag ] ?? '__return_empty_string' );
