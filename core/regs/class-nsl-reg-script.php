...
if ( ! class_exists( 'NSL_Reg_Script' ) ) {
	class NSL_Reg_Script implements NSL_Reg {
		const WP_SCRIPT = 'nsl-wp-script-generted';
		...
		public function __construct(
			...
			$this->src       = $src;
			$this->deps      = $deps;
			$this->ver       = is_null( $ver ) ? nsl()->get_version() : $ver;
		...
		public function register( $dispatch = null ) {
			if ( $this->handle && $this->src && ! wp_script_is( $this->handle, 'registered' ) ) {
				if ( self::WP_SCRIPT === $this->deps ) {
					...
					$path = path_join( dirname( nsl()->get_main_file() ), "assets/js/{$dir}/{$file}" );

					if ( file_exists( $path ) && is_readable( $path ) ) {
						$info = include $path;

						$this->src       = plugins_url( "assets/js/{$this->src}", nsl()->get_main_file() );
						$this->deps      = $info['dependencies'] ?? [];
						$this->ver       = $info['version'] ?? nsl()->get_version();
						$this->in_footer = true;
					}
				}
